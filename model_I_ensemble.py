from preprocessing import category_I_preprocessing
...
def main_13():
    ...
    _inputs, _outputs, input_dict, output_dict = category_I_preprocessing()

    # delete columns
    _inputs = deleteColumn(_inputs, [2, 3, 4, 5, 6])  # 요일, 사상자 수 제외
    ...
    while cnt < num_models:
        ...
        save model
        """
        model.save('./models/13_model_' + str(cnt) + '.h5')

        try:
            plot_model(model, to_file='./models/13_model_' + str(cnt) + '.png', show_shapes=True, show_layer_names=True)
        except:
            pass
    ...
    predict
    """
    f1 = open('./outputs/12_output.csv', 'r')
    f2 = open('./outputs/13_output.csv', 'w', newline='')
    r1 = csv.reader(f1)
    r2 = csv.writer(f2)
