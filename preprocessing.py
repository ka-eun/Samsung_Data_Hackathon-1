...
def category_preprocessing(file_train='./Kor_Train_교통사망사고정보(12.1~17.6).csv', file_test='./test_kor.csv'):
    f = open(file_train, 'r')
    r = csv.reader(f)
    ...
    for i, B in enumerate(output_set):
        for j, b in enumerate(B):
            if isinstance(b, int):
                ...
                dic_list[i][b] = oneHotEncoding(output_count[i],
                                                j)  # i번째 attribute에 해당되는 dictionary에서 키값을 b로 하는 벡터화된 value를 맵핑
    ...
    for i, row in enumerate(_output_train):
        output_train.append([])
        for j, elem in enumerate(row):
            output_train[i].append(dic_list[j][elem])
