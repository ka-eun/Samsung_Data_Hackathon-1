from preprocessing import category_N_preprocessing
...
def createModel(inputs, outputs):
    ...
    o1 = Activation('softmax', name='N')(x)
...
def main_5():
    ...
    _inputs, _outputs, input_dict, output_dict = category_N_preprocessing()

    # delete columns
    _inputs = deleteColumn(_inputs, [3, 13, 14])  # 사상자 수,  제외
    ...
    while cnt < num_models:
        ...
        save model
        """
        model.save('./models/5_model_' + str(cnt) + '.h5')

        try:
            plot_model(model, to_file='./models/5_model_' + str(cnt) + '.png', show_shapes=True, show_layer_names=True)
        except:
            pass
    ...
    predict
    """
    f1 = open('./outputs/4_output.csv', 'r')
    f2 = open('./outputs/5_output.csv', 'w', newline='')
    r1 = csv.reader(f1)
    r2 = csv.writer(f2)
