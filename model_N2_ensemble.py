...
def createModel(inputs, outputs):
    ...
    o1 = Activation('softmax', name='N2')(x)
...
def main_10():
    ...
    # delete columns
    _inputs = deleteColumn(_inputs, [0, 3, 4])  # 사상자 수,  제외
    ...
    while cnt < num_models:
        ...
        save model
        """
        model.save('./models/10_model_' + str(cnt) + '.h5')

        try:
            plot_model(model, to_file='./models/10_model_' + str(cnt) + '.png', show_shapes=True, show_layer_names=True)
        except:
            pass
    ...
    predict
    """
    f1 = open('./outputs/9_output.csv', 'r')
    f2 = open('./outputs/10_output.csv', 'w', newline='')
    r1 = csv.reader(f1)
    r2 = csv.writer(f2)
