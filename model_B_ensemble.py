from preprocessing import category_B_preprocessing
...
def createModel(inputs, outputs):
    ...
    o1 = Activation('softmax', name='B')(x)
...
def main_8():
    ...
    _inputs, _outputs, input_dict, output_dict = category_B_preprocessing()

    # delete columns
    _inputs = deleteColumn(_inputs, [2])  # 사상자 수,  제외
    ...
    while cnt < num_models:
        ...
        save model
        """
        model.save('./models/8_model_' + str(cnt) + '.h5')

        try:
            plot_model(model, to_file='./models/8_model_' + str(cnt) + '.png', show_shapes=True, show_layer_names=True)
        except:
            pass
    ...
    predict
    """
    f1 = open('./outputs/7_output.csv', 'r')
    f2 = open('./outputs/8_output.csv', 'w', newline='')
    r1 = csv.reader(f1)
    r2 = csv.writer(f2)
