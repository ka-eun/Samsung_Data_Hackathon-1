from preprocessing import category_A_preprocessing
...
def createModel(inputs, outputs):
    ...
    o1 = Activation('softmax', name='A')(x)
...
def main_12():
    ...
    _inputs, _outputs, input_dict, output_dict = category_A_preprocessing()

    # delete columns
    _inputs = deleteColumn(_inputs, [1, 2, 3])  # 사상자 수,  제외
    ...
    while cnt < num_models:
        ...
        save model
        """
        model.save('./models/12_model_' + str(cnt) + '.h5')

        try:
            plot_model(model, to_file='./models/12_model_' + str(cnt) + '.png', show_shapes=True, show_layer_names=True)
        except:
            pass
    ...
    predict
    """
    f1 = open('./outputs/11_output.csv', 'r')
    f2 = open('./outputs/12_output.csv', 'w', newline='')
    r1 = csv.reader(f1)
    r2 = csv.writer(f2)
