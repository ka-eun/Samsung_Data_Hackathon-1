from preprocessing import category_M_preprocessing
...
def createModel(inputs, outputs):
    ...
    o1 = Activation('softmax', name='M')(x)
...
def main_4():
    ...
    _inputs, _outputs, input_dict, output_dict = category_M_preprocessing()
    ...
    while cnt < num_models:
        ...
        save model
        """
        model.save('./models/4_model_' + str(cnt) + '.h5')

        try:
            plot_model(model, to_file='./models/4_model_' + str(cnt) + '.png', show_shapes=True, show_layer_names=True)
        except:
            pass
    ...
    predict
    """
    f1 = open('./outputs/3_output.csv', 'r')
    f2 = open('./outputs/4_output.csv', 'w', newline='')
    r1 = csv.reader(f1)
    r2 = csv.writer(f2)
    ...
    for i, row in enumerate(r1):
        ...
        if (20 < i & i < 31):
            ...
            # _inputs = deleteColumn(_inputs, [3, 12, 13, 14])  # 사상자 수,  제외
            for k, j in enumerate([3, 12, 13, 14]):
                del (_input_pred[j - k])
