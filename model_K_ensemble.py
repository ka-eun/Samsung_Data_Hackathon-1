from preprocessing import category_K_preprocessing
...
def createModel(inputs, outputs):
    ...
    o1 = Activation('softmax', name='K')(x)
...
def main_3():
    ...
    _inputs, _outputs, input_dict, output_dict = category_K_preprocessing()

    # delete columns
    _inputs = deleteColumn(_inputs, [1, 3])  # 요일, 사상자 수 제외
    ...
    while cnt < num_models:
        ...
        save model
        """
        model.save('./models/3_model_' + str(cnt) + '.h5')

        try:
            plot_model(model, to_file='./models/3_model_' + str(cnt) + '.png', show_shapes=True, show_layer_names=True)
        except:
            pass
    ...
    predict
    """
    f1 = open('./outputs/2_output.csv', 'r')
    f2 = open('./outputs/3_output.csv', 'w', newline='')
    r1 = csv.reader(f1)
    r2 = csv.writer(f2)

    for i, row in enumerate(r1):
        # blank
        if (10 < i & i < 21) | (40 < i & i < 46):
            """
            preprocessing
            """
            # copy
            ...
            # _row = deleteColumn(row, [1, 3])  # 요일, 사상자 수
            for k, j in enumerate([1, 3]):
                del (_input_pred[j - k])
