from preprocessing import category_O_preprocessing
...
def createModel(inputs, outputs):
    ...
    o1 = Activation('softmax', name='O')(x)
...
def main_6():
    ...
    _inputs, _outputs, input_dict, output_dict = category_O_preprocessing()

    # delete columns
    _inputs = deleteColumn(_inputs, [3, 14])  # 사상자 수,  제외
    ...
    while cnt < num_models:
        ...
        save model
        """
        model.save('./models/6_model_' + str(cnt) + '.h5')

        try:
            plot_model(model, to_file='./models/6_model_' + str(cnt) + '.png', show_shapes=True, show_layer_names=True)
        except:
            pass
    ...
    predict
    """
    f1 = open('./outputs/5_output.csv', 'r')
    f2 = open('./outputs/6_output.csv', 'w', newline='')
    r1 = csv.reader(f1)
    r2 = csv.writer(f2)

    for i, row in enumerate(r1):
        # blank
        if (20 < i & i < 24) | (26 < i & i < 31):
            """
            preprocessing
            """
            # copy
